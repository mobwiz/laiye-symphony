defmodule SymphonyElixir.Gitea.AdapterTest do
  ...
  test "client stops after a short Gitea candidate page" do
    first_page = [
      raw_issue(1),
    ...
    request_fun = fn "GET", "/repos/octo/repo/issues", params, nil, _settings ->
      ...
      if params["page"] == 1,
        do: {:ok, %{status: 200, body: first_page}},
        else: flunk("short pages must stop pagination")
    end
    ...
    refute_receive {:gitea_page, %{"page" => 2}}
  ...
  test "client fetches closed and combined states exactly" do
    for {states, query, expected_ids} <- [
      ...
      refute_receive {:gitea_state_page, ^query, %{"page" => 2}}
    end
  end
