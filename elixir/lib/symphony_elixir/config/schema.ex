defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Tracker do
    ...
    def changeset(schema, attrs) do
      ...
      |> update_change(:active_labels, fn labels ->
        labels
        |> Enum.map(&(String.trim(&1) |> String.downcase()))
        |> Enum.uniq()
      end)
  ...
  defp finalize_settings(settings) do
    ...
    {api_key, owner, assignee, provider, secret_environment_names} =
      case settings.tracker.kind do
        "linear" ->
          ...
          linear_provider =
            if is_nil(settings.tracker.owner),
              do: linear_provider,
              else: Map.put_new(linear_provider, "owner", settings.tracker.owner)
          ...
          resolved_owner =
            resolve_secret_setting(linear_provider["owner"], System.get_env("LINEAR_OWNER"))

          {
            ...
            resolved_assignee,
            linear_provider,
            ["LINEAR_API_KEY" | env_reference_names([linear_provider["api_key"], linear_provider["owner"]])]
          }

        _ ->
          {settings.tracker.api_key, settings.tracker.owner, settings.tracker.assignee, provider, []}
      end
